function Cart(localStorageKey){             //by using parameter localstoragekey we can use the cart function to different events with different parameter so 2 different carts will be created with their own products
    const cart={
        cartItems: undefined,
   
   
                   loadFromStorage(){
                       this.cartItems = JSON.parse(localStorage.getItem(localStorageKey));       //getting cart array from local storage , localstorage saves data in the form of string so we are using json.parse to convert it from string to html
               ...
               saveToStorage(){
                   localStorage.setItem(localStorageKey, JSON.stringify(this.cartItems));     //adding cart array to local storage so whenever we refresh the cart elements dont change
